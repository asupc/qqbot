...
async function showMsg() {
    ...
    ReturnMessage += `当前京豆：${$.beanCount}(即将过期${$.expirejingdou})京豆\n`;
    ...
    ReturnMessage += `[CQ:face,id=192]红包明细[CQ:face,id=192]`;
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.data.bizCode === 0) {
                            ...
                            if (data.data.result.factoryInfo) {
                                ...
                                infoMsg = `${name} 剩余${couponCount};电力投入${useScore}/${totalScore};当前总电力:${remainScore * 1 + useScore * 1} ;完成度:${((remainScore * 1 + useScore * 1) / (totalScore * 1)).toFixed(2) * 100}%`

                                if (((remainScore * 1 + useScore * 1) >= totalScore * 1 + 100000) && (couponCount * 1 > 0)) {
                                    // await jdfactory_addEnergy();
                                    infoMsg = `${name} ,目前数量:${couponCount},当前总电量为：${remainScore * 1 + useScore * 1},已经可以兑换：${totalScore},请去活动页面查看`
                                }
